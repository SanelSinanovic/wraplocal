...
function normalizeContractorProfile(row) {
  if (!row) return null
  const location = row.location || [row.city, row.state].filter(Boolean).join(', ')
  return {
    ...row,
    name: row.name || 'Contractor',
    title: row.title || 'Independent Installer',
    photo: row.photo_url || '',
    years: Number(row.years_experience) || 0,
    location,
    radius: row.service_radius || 'Local and regional projects',
    availability: row.availability || 'Available for projects',
...
function normalizeContractorThread(row) {
  if (!row) return null
  const companyName = row.company_name || row.shop?.name || row.company?.name || 'Company'
  return {
    ...row,
    contractor: normalizeContractorProfile(row.contractor),
    companyName,
    companyAvatar: row.shop?.avatar || companyName.split(' ').map(word => word[0]).join('').slice(0, 2).toUpperCase(),
    companyImage: row.shop?.banner_url || null,
  }
}
...
export async function fetchContractors() {
  ...
  if (error) { console.error('fetchContractors:', error); return { data: [], error } }
  return { data: (data || []).map(normalizeContractorProfile), error: null }
}

export async function fetchUserContractorProfile(ownerId) {
  const { data, error } = await supabase
    .from('contractor_profiles')
    .select('*')
    .eq('owner_id', ownerId)
    .maybeSingle()
  if (error) { console.error('fetchUserContractorProfile:', error); return { data: null, error } }
  return { data: normalizeContractorProfile(data), error: null }
}

export async function createContractorProfile({ ownerId, name, email = '' }) {
  const existing = await fetchUserContractorProfile(ownerId)
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      email,
      title: 'Independent Installer',
      availability: 'Available for projects',
      is_listed: true,
    })
    .select()
    .single()
  if (error) { console.error('createContractorProfile:', error); return { data: null, error } }
  return { data: normalizeContractorProfile(data), error: null }
}

export async function updateContractorProfile(profileId, fields) {
  const { data, error } = await supabase
    .from('contractor_profiles')
    .update(fields)
    .eq('id', profileId)
    .select()
    .single()
  if (error) { console.error('updateContractorProfile:', error); return { data: null, error } }
  return { data: normalizeContractorProfile(data), error: null }
}

const CONTRACTOR_THREAD_SELECT = '*, contractor:contractor_profiles(*), company:profiles!contractor_threads_company_id_fkey(id,name), shop:shops(id,name,avatar,banner_url,city,state)'

export async function createOrFetchContractorThread({ contractorId, companyId, shopId = null, companyName = '', subject = 'Contractor inquiry' }) {
  const existing = await supabase
    .from('contractor_threads')
    .select(CONTRACTOR_THREAD_SELECT)
    .eq('contractor_id', contractorId)
    .eq('company_id', companyId)
    .maybeSingle()
  if (!existing.error && existing.data) return { data: normalizeContractorThread(existing.data), error: null }

  const { data, error } = await supabase
    .from('contractor_threads')
    .insert({ contractor_id: contractorId, company_id: companyId, shop_id: shopId, company_name: companyName, subject })
    .select(CONTRACTOR_THREAD_SELECT)
    .single()
  if (error) {
    if (error.code === '23505') {
      const retry = await supabase
        ...
        .eq('company_id', companyId)
        .single()
      return { data: normalizeContractorThread(retry.data), error: retry.error }
    }
    console.error('createOrFetchContractorThread:', error)
    return { data: null, error }
  }
  return { data: normalizeContractorThread(data), error: null }
}

export async function fetchCompanyContractorThreads(companyId) {
  const { data, error } = await supabase
    .from('contractor_threads')
    .select(CONTRACTOR_THREAD_SELECT)
    .eq('company_id', companyId)
    .order('last_message_at', { ascending: false })
  if (error) { console.error('fetchCompanyContractorThreads:', error); return { data: [], error } }
  return { data: (data || []).map(normalizeContractorThread), error: null }
}

export async function fetchContractorThreads(contractorProfileId) {
  const { data, error } = await supabase
    .from('contractor_threads')
    .select(CONTRACTOR_THREAD_SELECT)
    .eq('contractor_id', contractorProfileId)
    .order('last_message_at', { ascending: false })
  if (error) { console.error('fetchContractorThreads:', error); return { data: [], error } }
  return { data: (data || []).map(normalizeContractorThread), error: null }
}

export async function fetchContractorThreadMessages(threadId) {
  const { data, error } = await supabase
    .from('contractor_messages')
    .select('*')
    .eq('thread_id', threadId)
    .order('sent_at', { ascending: true })
  if (error) { console.error('fetchContractorThreadMessages:', error); return { data: [], error } }
  return { data: data || [], error: null }
}

export async function sendContractorMessage({ threadId, senderId, senderRole, text }) {
  const { data, error } = await supabase
    .from('contractor_messages')
    .insert({ thread_id: threadId, sender_id: senderId, sender_role: senderRole, text })
    .select()
    .single()
  if (error) { console.error('sendContractorMessage:', error); return { data: null, error } }
  return { data, error: null }
}

export function subscribeToContractorMessages(threadId, callback) {
  return supabase
    .channel(`contractor-messages-${threadId}`)
    .on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
