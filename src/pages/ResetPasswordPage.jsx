...
export default function ResetPasswordPage({ nav, recoveryReady }) {
  ...
  return (
    <div style={{ fontFamily: "'Bebas Neue', cursive", background: "#0A0A0A", minHeight: "100vh", color: "#fff", display: "flex", flexDirection: "column" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@300;400;500;600&display=swap'); * { box-sizing: border-box; } input { width: 100%; background: #161616; border: 1px solid rgba(255,255,255,0.1); padding: 14px 16px; color: #fff; font-family: 'DM Sans', sans-serif; font-size: 15px; outline: none; transition: border-color 0.2s, background 0.2s; } input:focus { border-color: #FF4D00; background: #1c1c1c; } @keyframes fadeUp { from { opacity: 0; transform: translateY(24px); } to { opacity: 1; transform: translateY(0); } } @keyframes glowPulse { 0%,100% { opacity: 0.06; } 50% { opacity: 0.13; } } .reset-card { animation: fadeUp 0.5s cubic-bezier(0.22,1,0.36,1) both; } .reset-submit { background: #FF4D00; color: #fff; border: none; padding: 16px; font-family: 'Bebas Neue', cursive; font-size: 20px; letter-spacing: 3px; cursor: pointer; width: 100%; transition: background 0.2s, transform 0.15s; } .reset-submit:hover { background: #FF6A20; transform: translateY(-1px); } .reset-submit:active { transform: translateY(0); }`}</style>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 40, position: "relative", overflow: "hidden" }}>
        ...
        <div className="reset-card" style={{ width: "100%", maxWidth: 400, position: "relative", zIndex: 1 }}>
          ...
          {!recoveryReady ? (
            ...
            <>
              <div style={{ fontSize: 40, letterSpacing: 2, marginBottom: 8 }}>SET NEW PASSWORD</div>
