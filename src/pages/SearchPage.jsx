...
export default function SearchPage({ nav, searchQuery, setSearchQuery, setSelectedShop, setBookingShop }) {
  return (
    <div style={{ fontFamily: "'Bebas Neue', cursive", background: "#0A0A0A", minHeight: "100vh", color: "#fff" }}>
      ...
      <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 40px", background: "rgba(10,10,10,0.95)", borderBottom: "1px solid rgba(255,255,255,0.06)", position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 16, flex: 1, maxWidth: 500, margin: "0 40px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {SHOPS.map(shop => (
            <div key={shop.id} className="card-hover" onClick={() => { setSelectedShop(shop); nav("shop"); }}
              style={{ background: "#111", border: "1px solid rgba(255,255,255,0.06)", display: "flex", overflow: "hidden" }}>
              <img src={shop.image} alt={shop.name} style={{ width: 240, height: 160, objectFit: "cover", flexShrink: 0 }} />
              <div style={{ padding: "20px 24px", flex: 1, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>From</div>
                  <div style={{ fontSize: 32, color: "#FF4D00", marginBottom: 12 }}>${shop.price.toLocaleString()}</div>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: shop.availability === "Today" ? "#10B981" : "#F59E0B", marginBottom: 12 }}>● Available {shop.availability}</div>
                  <button className="btn-main" onClick={(e) => { e.stopPropagation(); setBookingShop(shop); nav("booking"); }}>Book Now</button>
                </div>
              </div>
            </div>
          ))}
