export default function ShopProfile({ nav, selectedShop, setBookingShop }) {
  ...
  return (
    <div style={{ fontFamily: "'Bebas Neue', cursive", background: "#0A0A0A", minHeight: "100vh", color: "#fff" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@300;400;500;600&display=swap'); * { box-sizing: border-box; } .btn-main { background: #FF4D00; color: #fff; border: none; padding: 14px 32px; font-family: 'Bebas Neue', cursive; font-size: 18px; letter-spacing: 2px; cursor: pointer; transition: all 0.2s; } .btn-main:hover { background: #FF6A20; } @media (max-width: 768px) { .shop-nav { padding: 12px 16px !important; } .shop-main-pad { padding: 0 16px 40px !important; } .shop-header { flex-direction: column !important; align-items: flex-start !important; gap: 16px !important; } .shop-title { font-size: 36px !important; } .shop-book-full { width: 100%; } .shop-content-grid { grid-template-columns: 1fr !important; } .portfolio-grid { grid-template-columns: repeat(2, 1fr) !important; } }`}</style>
      <nav className="shop-nav" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 40px", background: "rgba(10,10,10,0.95)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
      ...
      <div className="shop-main-pad" style={{ padding: "0 60px 60px", maxWidth: 1100, margin: "0 auto" }}>
          <div className="shop-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 40 }}>
            <div>
            <div className="shop-title" style={{ fontSize: 56, letterSpacing: 2 }}>{selectedShop.name}</div>
            ...
            <button className="btn-main shop-book-full" style={{ fontSize: 20, padding: "16px 40px" }} onClick={() => { setBookingShop(selectedShop); nav("booking"); }}>Book Appointment →</button>
        </div>

        <div className="shop-content-grid" style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 40 }}>
          <div>
